    ...
    while (true){
        ...
        if (((*unpack_write_index) != (*extractor_read_index))){
            if (!wait_for_next_iter){
                // if (*extractor_read_index + 1 >= BUFFER_SIZE){
                //     round_update_flag = true;
                // }
        ...
        if (has_unextract_flag){
            ...
            // while (!is_current_round(shm_p + 2048 + (cur_read_index * 2048) + 2, round_num)){
            //     struct timespec ts;
